...
file_path = r'B:/Clients/Ranger/Scheduling/Line Balancing.xlsm'
...
input_data = pd.read_excel(file_path, sheet_name='Input_Data',skiprows=3,usecols='B:C')
...
def import_data(file_path):
    df = pd.read_excel(file_path,sheet_name='Line_Details',skiprows=3,usecols='B:F')
...
def generate_assembly_line(file_path, env, feasable_solution, Workstation, que, switch):
    ...
    base = pd.read_excel(file_path,sheet_name='Line_Details',skiprows=3,usecols='B:G')
...
class Assembly_Line:
    ...
    def __init__(self, file_path, feasable_solution, Workstation, data):
        ...
        self.data = data
        self.file = pd.read_excel(file_path,sheet_name='Line_Details',skiprows=3,usecols='B:F')
...
def save_graph(data_set,workstation, shades):
    ...
    plt.savefig('Allocated_Ws.png', dpi=300)
